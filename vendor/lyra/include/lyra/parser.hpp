...
class parser
{
	...
	virtual std::string get_group_text(const option_style &) const
	{
		return {};
	}
		...
		printer & p, const option_style & style) const
	{
		...
		if (!usage_text.empty())
		{
			auto title = get_group_text(style);
			if (!title.empty()) title += ' ';
			p.paragraph(style, usage_text, "Usage: " + title);
		}
...
{
	std::string suffix = std::move(m_description);
	m_description = help_description_text;
	bool dot_needed = !m_description.empty() && m_description.back() == '.';
	if (dot_needed) m_description.pop_back();
	if (!suffix.empty()) m_description += " " + suffix;
	if (dot_needed) m_description += ".";
